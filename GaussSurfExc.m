function [fxc, fxs, fyc, fys, fzc, fzs, rho, z] = GaussSurfExc(rhofield, zfield, NA, fd, n0, n, n1, d0, d, d1, lamex, over, focpos, atf, resolution, ring, aberration, maxm)
                                                           
% function [fxc, fxs, fyc, fys, fzc, fzs, rho, z] = GaussSurfExc(rhofield,
% zfield, NA, fd, n0, n, n1, d0, d, d1, lamex, over, focpos, atf,
% resolution, ring, maxm) calculates the surface electric field distribution of a 
...
maxnum = 1e3;
if nargin < 18 | isempty(maxm)
    maxm = 2;
end
if nargin < 17 | isempty(aberration)
    aberration = 0;
end
...
rad = s0/max(s0);
fac = fac.*exp(i*k0*aberration*(6*rad.^2.*(rad.^2-1)+1));
...
phase1 = - k0(1)*c0*(focpos(1)-zv);
phase2 = k*cc*ones(size(zv))*2*d - k0(1)*c0*(focpos(1)-zv);
ez1 = exp(i*phase1);
ez2 = exp(i*phase2);
