...
if (isfield(res, 'metadata') && isfield(res.metadata, 'NumLasers') && (res.metadata.NumLasers == 1))
    % 1 Laser Pulse only
    
    if (size(res.auto, 2) == 4)
        % blue ACF
        y(:,1,:) = res.auto(ind,3,4,:) + res.auto(ind,4,3,:);
        % red ACF
        y(:,2,:) = res.auto(ind,1,2,:) + res.auto(ind,2,1,:);
    elseif (size(res.auto, 2) == 2)
        y(:,1,:) = res.auto(ind,1,2,:) + res.auto(ind,2,1,:);
    else
        y(:,1,:) = res.auto(ind,1,1,:) + res.auto(ind,1,1,:);
    end
elseif (size(res.auto, 2) == 16)        
...
elseif (size(res.auto, 2) == 2) 
    % 1 Laser -> 2 Detector
    y(:,1,:) = squeeze(res.auto(ind,1,1,:));
    y(:,2,:) = squeeze(res.auto(ind,2,2,:));
    y(:,3,:) = squeeze(res.auto(ind,1,2,:));
    y(:,4,:) = squeeze(res.auto(ind,2,1,:));
elseif (size(res.auto, 2) == 1) 
    y(:,1,:) = squeeze(res.auto);
    y(:,2,:) = squeeze(res.auto);
    y(:,3,:) = squeeze(res.auto);
    y(:,4,:) = squeeze(res.auto);
else
    % 2 Laser -> 2 Detector
    L1 = 1;
    L2 = 2;
    ...
    y(:,1,:) = res.auto(ind,L1+D1,L1+D2,:) + res.auto(ind,L1+D2,L1+D1,:);
    y(:,2,:) = res.auto(ind,L2+D1,L2+D2,:) + res.auto(ind,L2+D2,L2+D1,:);
    y(:,3,:) = res.auto(ind,L1+D1,L2+D2,:);
    y(:,4,:) = res.auto(ind,L2+D1,L1+D2,:);
    y(:,5,:) = res.auto(ind,L1+D2,L2+D1,:);
    y(:,6,:) = res.auto(ind,L2+D2,L1+D1,:);
end
