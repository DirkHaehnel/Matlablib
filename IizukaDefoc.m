function [res, z, zz, a, b, model] = IizukaDefoc(file,pic,roi,model)
...
mag = 350; % magnification
focus = 0.96; % defcousing in mum
...
tmp = dir([outfile '*.png']);
if ~isempty(tmp)>0
    outfile = [outfile mint2str(str2double(tmp(end).name(end-10:end-8))+1,3)];
else
