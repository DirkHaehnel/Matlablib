...
%   convergence, FLAG = 1 not.
%
%   If B is not specified, B = eye(size(A)) is used.
%
...
%   "shift", the K-th eigenvalues nearest SIGMA are computed.  If SIGMA
%   is one of the following strings, it specifies the desired eigenvalues.
%   
...
%   $Revision: 1.12 $  $Date: 1997/04/08 06:37:31 $
...
if isempty(options)
   ...
   fopts = fields(options);
end

% A is the matrix of all zeros (not detectable if A is a string)

if ~isstr(A)
   if isequal(A,spalloc(n,n,1)) | isequal(A,zeros(n,n))
      if nargout < 2
         v = zeros(k,1);
      else
         v = eye(n,k);
...
elseif cheb
   warning('Cheb option only available when issym = 1.  Proceeding without cheb.')
   cheb = 0;
end
...
if stopcrit <= tol
   ...
   if nargout < 3 & dispn ~= 0
      if iter >= maxit
         ...
         disp('Eigenvalues did not converge')        
      end
   else
      flag = 1;
   end
