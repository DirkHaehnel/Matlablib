function [y, tcspc, flag, num, overcount, head] = ingopt3Read(name, start_cnt, cnts, start_time, sync)

%        [y, tcspc, flag, num, overcount, head] = ingopt3Read(name, start_cnt, cnts, start_time, sync);



if strcmp(name(end-3:end),'.ingopt3')
    
    hlength = 0;
    
    if nargin>1
        if (nargin<5)||(nargout==6) 
            [head, hlength] = ingopt3Read_head(name);
