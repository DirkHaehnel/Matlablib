...
for j=1:mm
    ...
    p = Simplex('ExpFun',[1e-6 1e-5],[0 0],[],[],[],res.autotime(ind),sum(sum(res.auto(ind,:,:),3),2),1);
    %p = Simplex('ExpFun',[1e-5],[0],[],[],[],res.autotime(ind),sum(sum(res.auto(ind,:,:),3),2),1);
    for k=1:2
        p = Simplex('ExpFun',p,0*p,[],[],[],res.autotime(ind),sum(sum(res.auto(ind,:,(j-1)*ng+1:min([j*ng,end])),3),2),1);
    end
    p = 1./[1e4 1e5]';
    tst = simplex('Rigler',[4e-2 2e-3 1./p'],[0 0 1./p'],[inf inf 1./p'],[],[],res.autotime(2:end),sum(sum(res.auto(2:end,:,(j-1)*ng+1:min([j*ng,end])),3),2),[],1);
    for k=1:2
...
if flag
    ...
    pp0 = simplex('AntiBunch',[0 3 2 7 2],[],[],[],[],anti.autotime(ind),sum(anti.auto(ind,:,:),3),25,1,1);
    for k=1:2
        pp0 = simplex('AntiBunch',pp0,[],[],[],[],anti.autotime(ind),sum(anti.auto(ind,:,:),3),25,1);
    end
    for j=1:mm
        pp = simplex('AntiBunch',pp0,[],[],[],[],anti.autotime(ind),sum(anti.auto(ind,:,(j-1)*ng+1:min([j*ng,end])),3),25,1);
        for k=1:2
            pp = simplex('AntiBunch',pp,[],[],[],[],anti.autotime(ind),sum(anti.auto(ind,:,(j-1)*ng+1:min([j*ng,end])),3),25,1);
        end
        [err,cc(:,j),c0(:,j),z,z,z] = AntiBunch(pp,anti.autotime(ind),sum(anti.auto(ind,:,(j-1)*ng+1:min([j*ng,end])),3),25,1);
    end
end
