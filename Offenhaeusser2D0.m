...
h = 2e-5; % height of gap = FE cell size for integrating the Poisson equation
delta = 2e-5; % FE cell size for integrating the Nernst-Planck equations, should be an odd multiple of h
...
RM = fac*[-DifK*cK0*[1 1 -1]; -DifNa*cNa0*[1 1 -1]; DifCl*cCl0*[1 1 -1]];
RM = expm(RM)';
...
dt = min(0.2*delta^2./[DifK DifNa DifCl]); % time step in sec
DifK = DifK/delta^2; 
DifNa = DifNa/delta^2; 
DifCl = DifCl/delta^2; 
kappa = kappa*dt/h;
...
dstep = 0.1; % max increment per integration step
...
for j=1:ceil(Tmax/dt)
    ...
    while ddt<dt % integration with self-regulating time step
        fm = rho2.*exp(-fac2.*psi);
        Wp = (L*fm)./fm;
        fp = rho2.*exp(fac2*psi);
        Wm = (L*fp)./fp;
        % V = -ElectronCharge/Temperature/BoltzmannConstant*FaradayConstant*4*pi*netCharge./diel;
        V = ElectronCharge/Temperature/BoltzmannConstant*(L*(rho2.*psi) - psi.*rho2L)./rho2;
        cKTilde = fp.*cK;
        cNaTilde = fp.*cNa;    
        cClTilde = fm.*cCl;    
        ...
        % tmp = max(-((cKstep(~netCharge==0)+cNastep(~netCharge==0))./fp(~netCharge==0) - cClstep(~netCharge==0)./fm(~netCharge==0))./netCharge(~netCharge==0));
...
%         tst = ind(netCharge<1e-20); %*max(abs(cK)+abs(cNa)+abs(cCl)));
%         tmp = [cK(tst) cNa(tst) cCl(tst)]*RM;
%         cK(tst) = tmp(:,1);
%         cNa(tst) = tmp(:,2);
%         cCl(tst) = tmp(:,3);
%         netCharge(tst) = 0;
