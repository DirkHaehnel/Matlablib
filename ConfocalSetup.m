...
surf(-rad*[1;1]*cos(phi), -rad*[1;1]*sin(phi), [-4*height*ones(1,length(phi)); -6*height*ones(1,length(phi))], 0.8*ones(2,length(phi)), 'facealpha', lighttransparency)
...
% Beam Splitter
surf(-rad*[1;1]*cos(phi), [+sin(phi)*rad; height*ones(1,length(phi))], -5*height-rad*[1;1]*sin(phi), 0.8*ones(2,length(phi)), 'facealpha', lighttransparency)
surf([0 len], [-len len], -5*height+[1 1; -1 -1]*len, zeros(2,2),'facealpha', 0.1) 
surf([0 len], [-len len], -5*height-foc+[1 1; -1 -1]*len, zeros(2,2),'facealpha', 0.1)         
surf([0 len; 0 len], -len*ones(2), -5*height+len+[-foc -foc; 0 0], zeros(2,2),'facealpha', 0.3) 
surf(zeros(2), -len*[1 -1; 1 -1], -5*height+[-foc+len -foc-len; len -len], zeros(2,2),'facealpha', 0.3) 
...
rho = (0:50)'/50*rad;
lens = 1.5e3;
surf(-rho*cos(phi), -rho*sin(phi), -6*height-(sqrt(lens^2-rad^2)-sqrt(lens^2-rho.^2))*ones(1,length(phi)), 0*ones(length(rho),length(phi)), 'facealpha', 0.4)
surf(-rho*cos(phi), -rho*sin(phi), -6*height+(sqrt(lens^2-rad^2)-sqrt(lens^2-rho.^2))*ones(1,length(phi)), 0*ones(length(rho),length(phi)), 'facealpha', 0.4)

% 4. Lens
rho = (0:50)'/50*rad;
lens = 1.5e3;
surf(-rho*cos(phi), height-(sqrt(lens^2-rad^2)-sqrt(lens^2-rho.^2))*ones(1,length(phi)), -5*height-rho*sin(phi),  0*ones(length(rho),length(phi)), 'facealpha', 0.4)
surf(-rho*cos(phi), height+(sqrt(lens^2-rad^2)-sqrt(lens^2-rho.^2))*ones(1,length(phi)), -5*height-rho*sin(phi), 0*ones(length(rho),length(phi)), 'facealpha', 0.4)

% Detectors
surf(-rad*[1;0]*cos(phi), -rad*[1;0]*sin(phi), -6*height-[0;5*foc]*ones(1,length(phi)), 0.8*ones(2,length(phi)), 'facealpha', lighttransparency)
...
surf(-rad*[0.4;0]*cos(phi), -rad*[0.4;0]*sin(phi), -6*height-5*foc*ones(2,length(phi)), 0.*ones(2,length(phi)))
surf(-rad*[0.4;0]*cos(phi), height+5*foc*ones(2,length(phi)), -5*height-rad*[0.4;0]*sin(phi), 0.*ones(2,length(phi)))
