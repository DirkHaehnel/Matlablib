...
for m1=1:6
    Tmax = 0.75; % max integration time in sec
    rad = (15+m1)*1e-4; % radius of cell
    ...
    h = 5e-6; % height of gap = FE cell size for integrating the Poisson equation
    ...
    kappa = 1/2.5e7*Coulomb/Volt/FaradayConstant/(3*pi*rad^2); % statC/statV/cm^2/s
    kap(1:100)=0;
    kap(5:40)=kappa;
    % internal potential
    ...
    for j=1:ceil(Tmax/ddt)
        ...
        for k=1:round(ddt/dt)
            % K-influx:
            %         cK(gap) = cK(gap) + kap(j)*V0/h;
            %         cK(gap) = cK(gap) + kappa*V0/h;
            eag_slope=(eagIvst(ceil((j+1)/0.99),2)-eagIvst(ceil(j/0.99),2))/(eagIvst(ceil((j+1)/0.99),1)-eagIvst(ceil(j/0.99),1));
            eag=eagIvst(ceil(j/0.99),2)+eag_slope*(j/eagIvst(ceil(j/0.99),1));
            cK(gap) = cK(gap) + eag*dt*Coulomb*1e-9/FaradayConstant/(3*pi*rad^2*h);
        ...
        for m=1:length(gap)
            intK(j,m1)=intK(j,m1) + 1/(pi*rad.^2*h)*(resK(m,j)*(pi*h*((m*delta).^2-((m-1)*delta).^2)));
        end
        subplot(411); plot(1e4*rho(bottom), 1e6*cK(bottom));     
        title(['time = ' num2str(1e3*t(end)) ' ms']);     
        ylabel('K^+ [mM]')
        subplot(412); plot(1e4*rho(bottom), 1e6*cCl(bottom)); 
        ylabel('Cl^- [mM]')
        subplot(413); plot(1e4*rho(bottom), 1e6*cNa(bottom)); 
        ylabel('Na^+ [mM]')    
        %     xlabel('radius [\mum]');
        subplot(414);plot(1e6*intK);
        ylabel('mean K^+ [mM]');
        xlabel('time [ms]');
        drawnow; 
