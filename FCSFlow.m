function [modres, autotime] = FCSFlow(rho,z,volx1,voly1,volx2,voly2,d,velo)
...
for jj=0:4*maxm
    ...
    x(:,:,jj) = cos(psi)*rho(:,1)*ones(1,size(z,2));
    y(:,:,jj) = sin(psi)*rho(:,1)*ones(1,size(z,2));    
    f1(:,:,jj) = volx1(:,:,1)+voly1(:,:,1);
    f2(:,:,jj) = volx2(:,:,1)+voly2(:,:,1);
	for kk = 1:maxm 
        f1(:,:,jj) = f1(:,:,jj) + (volx1(:,:,kk+1)+voly1(:,:,kk+1))*cos(kk*phi(jr)) + ...
            (volx1(:,:,maxm+1+kk)+voly1(:,:,maxm+1+kk))*sin(kk*phi(jr));
        f2(:,:,jj) = f2(:,:,jj) + (volx2(:,:,kk+1)+voly2(:,:,kk+1))*cos(kk*phi(jr)) + ...
            (volx2(:,:,maxm+1+kk)+voly2(:,:,maxm+1+kk))*sin(kk*phi(jr));
    end
end
z = repmat(z,[size(z,1) 1 1]);
