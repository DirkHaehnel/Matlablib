function [volx, voly, rho, z, fxc, fxs, fyc, fys, fzc, fzs, qv, qp] = OneFocusMDF(rhofield, zfield, NA, fd, n0, n, n1, d0, d, d1, lamex, over, focpos, lamem, mag, av, zpin, atf, kappa, lt, sat, resolution, ring, maxm)
...
if sat>0
    ...
    % tmp = sqrt(tmp);
    % fxc1 = fxc1/tmp; fxs1 = fxs1/tmp;
    % fyc1 = fyc1/tmp; fys1 = fys1/tmp;
    % fzc1 = fzc1/tmp; fzs1 = fzs1/tmp;
    % fxc2 = fxc2/tmp; fxs2 = fxs2/tmp;
    % fyc2 = fyc2/tmp; fys2 = fys2/tmp;
    % fzc2 = fzc2/tmp; fzs2 = fzs2/tmp;
    sat = sat/tmp; % normalization of optical saturation to maximum of intensity excitation distribution
end

volx = zeros(size(rho,1),size(rho,2),2*maxm+1); voly = volx;
for j=0:2*maxm
    psi = j/(2*maxm+1)*2*pi;
