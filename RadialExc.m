function [fxc, fxs, fyc, fys, fzc, fzs, rho, z] = RadialExc(rhofield, zfield, NA, fd, n0, n, n1, d0, d, d1, lamex, over, focpos, atf, resolution, ring, maxm)
...
if nargin>15 && ~isempty(ring) % ring has to be a function of rad and psi
    fun = ['fac.*exp(-om(1)*(fd*n*ss*cp-focpos(4)).^2 - om(2)*(fd*n*ss*sp-focpos(5)).^2 + i*k*ss*rho0*cos(psi-psi0) + (' ring '));'];
else
    fun = 'fac.*exp(-om(1)*(fd*n*ss*cp-focpos(4)).^2 - om(2)*(fd*n*ss*sp-focpos(5)).^2 + i*k*ss*rho0*cos(psi-psi0));';
    ring = [];
end
for j=0:2*maxm
    psi = j/(2*maxm+1)*2*pi; cp = cos(psi); sp = sin(psi);
    eval(['ef = ' fun]);
    tmpxt(:,j+1) = (cp*tp.*cc).*ef;
    tmpxr(:,j+1) = (-cp*rp.*cc).*ef;
    tmpyt(:,j+1) = sp*(tp.*cc).*ef;
    tmpyr(:,j+1) = sp*(-rp.*cc).*ef;
    tmpzt(:,j+1) = -ss.*tp.*ef;
    tmpzr(:,j+1) = -ss.*rp.*ef;
end
