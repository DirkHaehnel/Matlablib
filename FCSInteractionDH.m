function auto = FCSInteractionDH(p,autotime,rho,z,u)
...
qv = (0.5:1e3)/1e3*pi/(200*mean(diff(z(1,:))));
...
for j = 1:length(autotime)
    ...
    rt = 0*r0;
    ind = eir0+autotime(j)<=max(eir0);
    rt(ind) = interp1(eir0(ind),r0(ind),eir0(ind)+autotime(j),'cubic');
    rt(~ind) = max(r0);
    ...
    rr(rad<min(rt(:))) = 0*min(rt(:));
    rr(rad>max(rt(:))) = max(rt(:));    
    
    V = exp(-p(2)*fun(rr/p(1)));
    ...
    %mpcolor(qv,kv,abs(Vwave).^2); drawnow
    auto(j) = real(sum(sum((kv'*ones(size(qv))).*Vwave.*uwave.*exp(-kk*autotime(j)))));
