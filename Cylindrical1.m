function [coef, M, V] = Cylindrical1(wv,mv,nv,rv,c0)
    ...
    if len==1
        ...
        for j=1:length(mv)        
            ...
            for kk=length(nv)-1:-1:3
                ...
                renj = exp(-imag(q2*diff(rv(kk-1:kk))));       
                renh = exp(-i*(q2*diff(rv(kk-1:kk))));       
                M(1+4*cnt:4*(cnt+1),-2+4*cnt+(1:8)) = ...
                    [renj*q2.*bjx(m,q2*r), renj*m*w/r.*bj(m,q2*r), renh*q2.*bhx(m,q2*r), renh*m*w/r.*bh(m,q2*r), -q1.*bjx(m,q1*r), -m*w/r.*bj(m,q1*r), -q1.*bhx(m,q1*r), -m*w/r.*bh(m,q1*r); ...
                        0, -renj*q2.^2.*bj(m,q2*r), 0, -renh*q2.^2.*bh(m,q2*r), 0, q1.^2.*bj(m,q1*r), 0, q1.^2.*bh(m,q1*r); ...
                        renj*m*w/r.*bj(m,q2*r), renj*n2^2*q2.*bjx(m,q2*r), renh*m*w/r.*bh(m,q2*r), renh*n2^2*q2.*bhx(m,q2*r), -m*w/r.*bj(m,q1*r), -n1^2*q1.*bjx(m,q1*r), -m*w/r.*bh(m,q1*r), -n1^2*q1.*bhx(m,q1*r);
                    -renj*q2.^2.*bj(m,q2*r), 0, -renh*q2.^2.*bh(m,q2*r), 0, q1.^2.*bj(m,q1*r), 0, q1.^2.*bh(m,q1*r), 0 ...
                    ];
            ...
            renj = exp(-imag(q2*diff(rv(1:2))));
            renh = exp(-i*(q2*diff(rv(1:2))));
            M(end-3:end,end-5:end) = [renj*q2.*bjx(m,q2*r), renj*m*w/r.*bj(m,q2*r), renh*q2.*bhx(m,q2*r), renh*m*w/r.*bh(m,q2*r), -q1.*bjx(m,q1*r), -m*w/r.*bj(m,q1*r); ...
                    0, -renj*q2.^2.*bj(m,q2*r), 0, -renh*q2.^2.*bh(m,q2*r), 0, q1.^2.*bj(m,q1*r); ...
                    renj*m*w/r.*bj(m,q2*r), renj*n2^2*q2.*bjx(m,q2*r), renh*m*w/r.*bh(m,q2*r), renh*n2^2*q2.*bhx(m,q2*r), -m*w/r.*bj(m,q1*r), -n1^2*q1.*bjx(m,q1*r);
                -renj*q2.^2.*bj(m,q2*r), 0, -renh*q2.^2.*bh(m,q2*r), 0, q1.^2.*bj(m,q1*r), 0 ...
                ];
