function [ux, uy, pp, po, op] = GaussExc2RotoFCS(exc, lamem, mag, av, zpin, atf, gamma)
...
[m0, m1, m2, m3, m4] = RotoDiffCoef(gamma);
mm = 0*m4;
for L=0:4
...
for jpsi=0:2*maxm
    ...
    for L = 0:4
        for M = 0:L
            for K = -L:L
                pp(L+1,M+1) = pp(L+1,M+1) + (1+sign(M))*sum(rho(:,1)'*real(poyntx(:,:,L+1,M+1,L+K+1).*conj(poynty(:,:,L+1,M+1,L+K+1))));
                po(L+1,M+1) = po(L+1,M+1) + (1+sign(M))*sum(rho(:,1)'*real(poyntx(:,:,L+1,M+1,L+K+1).*conj(poyntx(:,:,L+1,M+1,L+K+1))));
