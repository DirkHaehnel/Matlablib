function [G,l]=gaunt(l1,l2,l3,m1,m2,m3,meth)
% [G,l]=GAUNT(l1,l2,l3,m1,m2,m3,meth)
%
...
%            'table'     Table look-up prestored from recursive (THREEJ)
...
%
% Last modified by fjsimons-at-alum.mit.edu, 11.08.2006
  ...
  switch meth
   ...
   case 'gl'
	...
	['(-1)^%i*xlm(%i,%i,acos(x)).*'...
	 'xlm(%i,%i,acos(x)).*'...
	 'xlm(%i,%i,acos(x))'],...
	 m1,l1,-m1,l2,m2,l3,m3));
    % And the exponential evaluates to 2*pi
    G=gausslegendre([-1 1],integrand,l1+l2+l3)*2*pi;
    l=l1;
   case 'table'
    % Note that WIGNER0J would use a different algorithm
    w0=threej(l1,l2,l3,0,0,0);
