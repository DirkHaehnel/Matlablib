function [res, head] = pt3ProOne(name, flag, param, weight);
...
if strcmp(flag,'xfcs')
    ...
    while (tst>0)
        ...
        [head, tmpy, tmpx, z, tst, overcount] = pt3Read_t(name, cnt+1, overcount, max_time, CntRate0);
        ...
        if (tst>0)
            ...
            tcspc = tcspc + mHist(tmpx, tau);

            if tmpy(end)>z32
                auto(:,1) = auto(:,1) + pt32fcs(tmpy, double(ind==1), double(ind==1), Ncasc, Nsub, smooth);
            else
                auto(:,1) = auto(:,1) + tttr2fcs(tmpy, double(ind==1), double(ind==1), Ncasc, Nsub, smooth);
            end
