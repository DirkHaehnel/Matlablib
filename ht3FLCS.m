function[autotime auto automean autofinal master] = ht3FLCS( name, param)
...
%                  dnum = length(dind);
%                  
%                      tcspcirf = zeros(NChannelsirf,dnum);
%                  
%                     for j=1:length(dind)
%                          rev_dind(dind(j)+1) = j;
%                     end
...
%                           tcspcirf(tmpx(j)+1, dd) = tcspcirf(tmpx(j)+1, dd) + 1;
...
k=size(param,2)/det;
    ...
    for i=1:det;
        c=c+1;
        l=1+(c-1)*k;
        master = [master, ((param(:,l:c*k)'*diag(1./tcspc(:,c))*param(:,l:c*k))\(diag(1./tcspc(:,c))*param(:,l:c*k))')'];
    end
    time = 0;
    cnt = 0;
    tst = 1; 
...
automean=mean(auto,4);
  ...
  for s=1:size(master,2)/det;
      ...
      autofinal(:,s)=autofinal(:,s)-min(autofinal(:,s));
