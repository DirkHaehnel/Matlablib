function [res, head] = OneFocus2FCS(name, maxtime, autobin, filt)
...
while tst>0
    ...
    elseif strcmp(name(end-2:end),'ht3')
        [tmpy, tmpx, flv, special, num, overcount, head] = ht3read(name, [cnt+1, photons]);
    end
    cnt = cnt+tst;
    if tst>0
    ...
    while tst>0
        ...
        if tst>0
            ...
            if autobin>=1
                ...
                auto(:,:,:,jj) = tttr2xfcs(round((tmpy*head.Sync+tmpx*head.Resolution)/head.Resolution), flvp, Ncasc, Nsub);
            end
