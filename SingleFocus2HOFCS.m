function [res, head] = SingleFocus2HOFCS(name, order, timebin)
...
auto = zeros(length(autotime),order,order,2,2);
intensity = zeros(order,2);
...
while tst>0
    ...
    if tst>0
        cnt = cnt + tst;
        dt = tmpy(end)-tmpy(1);
        time = time + dt/timebin;
        rate = [rate tst/dt/timeunit];        
        tcspc(:,1) = tcspc(:,1) + mhist(tmpx(flv==0), 1:head.NChannels);
        tcspc(:,2) = tcspc(:,2) + mhist(tmpx(flv==1), 1:head.NChannels);            
        for p=1:2
            flvp = flv==p-1;
            ...
            for r=1:order
                intensity(r,p) = intensity(r,p) + sum(flvp.^r);
                for s=1:order
                    auto(:,r,s,p,p) = auto(:,r,s,p,p) + tttr2fcs(tmp, flvp.^r, flvp.^s, Ncasc, Nsub, 1);
                end
            end
        end
        for p=1:2
            for q=(p+1):2
                ...
                for r=1:order
                    for s=1:order
                        auto(:,r,s,p,q) = auto(:,r,s,p,q) + tttr2fcs(tmp, flvp.^r, flvq.^s, Ncasc, Nsub, 1);
                        auto(:,r,s,q,p) = auto(:,r,s,q,p) + tttr2fcs(tmp, flvq.^r, flvp.^s, Ncasc, Nsub, 1);
                    end
                end
                ...
                semilogx(autotime, auto(:,1,1,p,q)/time); 
                drawnow;
            end
        end
    end
...
res = setfield(res, 'auto', squeeze(auto)/time);
res = setfield(res, 'intensity', intensity/time);
tmp = 1:head.NChannels;
res = setfield(res, 'tau', head.Resolution*tmp(sum(tcspc')>0));
