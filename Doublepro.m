...
% flag 6 = fitting the BSD to the experimental results with different Lee filters
% flag 7 = draws BSDs of lowest pressure measurement for different Lee filter width
...
if flag==5 % bsd fitting
   ...
   c = dave(t, 1)'/bs(t);
	t = 1:100;
	plot(t, c*bs(t), t, dave(t, 1), 'og', 'Markersize', 4);
	set(gca, 'ytick', []); times16;
	xlabel('Burst size (photoelectrons)', 'FontName', 'Times', 'FontSize', 16); ylabel('Relative frequency', 'FontName', 'Times', 'FontSize', 16); drawnow
	t = 15:len;
	res = []; c = [];
	for j = 1:9
		kappa = simplex('doubfit', 0.5, 0, [], [], [], dave(t, j)', bb(:, t));
		res = [res kappa];
		tmp = exp((0:(mx-1))*log(1-kappa))*kappa^2;
		c = [c; (dave(t, j)'/(tmp*bb(:, t)))*tmp];
	end
	kappa = res;
	j = 9;
	figure
	h = axes;
	plot(1:len, c(j,:)*bb, 15:max(t), dave(15:max(t), j), 'og', 'Markersize', 4);
	hold
	for k = 1:mx
	...
	xlabel('Pressure (mm Hg)', 'FontName', 'Times', 'FontSize', 16); ylabel('Ratio tau_b/tau_m', 'FontName', 'Times', 'FontSize', 16); times16
	axis([19 200 0 1.201])
end

if flag==6 % bsd fitting for differetn widths of the Lee filter
   load leetst1;
   for j=3:7
      eval(['res', num2str(j), '=bsd', num2str(j), '/sum(bsd', num2str(j),');']);
   end
   for k=2:9 
      eval(['load leetst', num2str(k)]);
      for j=3:7
         tmp = num2str(j);
         eval(['res', tmp, '=[[res', tmp, ', zeros(size(res', tmp, ',1), length(bsd', tmp, ')-size(res', tmp, ',2))]; [bsd', tmp, '/sum(bsd', tmp, '), zeros(1, size(res', tmp, ',2)-length(bsd', tmp, '))]];']);
      end
   end
   ...
   t = 15:len;
	pressure=[35 40 55 70 90 110 135 170 200];
   count = 3;
   eval(['dave=res', num2str(count), ';']);
   res = []; c = [];
   for j = 1:9
	kappa = simplex('doubfit', 0.5, 0, [], [], [], dave(j, t), bb(:, t));
		res = [res kappa];
		tmp = exp((0:(mx-1))*log(1-kappa))*kappa^2;
		c = [c; (dave(j, t)/(tmp*bb(:, t)))*tmp];
	end
   kappa = res;
	figure
   fac = -log(kappa)/[ones(1,9); pressure];
...
if flag==7 % draws BSDs of lowest pressure measurement for different Lee filter width
   load leetst1;
   t = 7:length(bsd0);
   plot(t,bsd0(t),t,bsd1(t),t,bsd2(t),t,bsd3(t),t,bsd4(t),t,bsd5(t),'LineWidth',2);
	text(10,100,'m=0');
   text(6,38,'m=1');
   text(6,6,'m=2,3,4,5');
   times16;
