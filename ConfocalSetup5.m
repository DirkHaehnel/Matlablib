% figure for 2color-2focus + optical tweezer confocal setup
...
% Upper cover glass
surf([0 len], [-len len], foc*d*ones(2,2), zeros(2,2),'facealpha', 0.1) 
surf([0 0; len len], -len*ones(2,2), foc*[1 d; 1 d], zeros(2,2),'facealpha', 0.3) 
surf(zeros(2,2), [-len -len; len len], foc*[1 d; 1 d], zeros(2,2),'facealpha', 0.2)

% water layer
surf([0 0; len len], -len*ones(2,2), foc*[-1 1; -1 1], zeros(2,2),'facealpha', 0.1) 
surf(zeros(2,2), [-len -len; len len], foc*[-1 1; -1 1], zeros(2,2),'facealpha', 0.1)
...
surf(-rad*[1;1]*cos(phi), -3*height-rad*[1;1]*sin(phi), [-2*height-rad*sin(phi); -3.5*height*ones(1,length(phi))], 0.2*ones(2,length(phi)), 'facealpha', lighttransparency)
surf(-rad*[1;1]*cos(phi), [-3*height-rad*sin(phi); -4.5*height*ones(1,length(phi))], -2*height-rad*[1;1]*sin(phi), 0.8*ones(2,length(phi)), 'facealpha', lighttransparency)
surf(-rad*[1;1]*cos(phi), [-sin(phi)*rad-1.5*height*ones(1,length(phi)); -sin(phi)*rad-3*height*ones(1,length(phi))], -2*height-rad*[1;1]*sin(phi), 0.5*ones(2,length(phi)), 'facealpha', lighttransparency)
...
surf(-rad*[1;1]*cos(phi), -1.5*height-rad*[1;1]*sin(phi), [-2*height-rad*sin(phi); -3.5*height*ones(1,length(phi))], 1*ones(2,length(phi)), 'facealpha', lighttransparency)
